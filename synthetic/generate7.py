...
def save_data(data, batch_count, format='json'):
    ...
    if format == 'csv' or format == 'both':
        ...
        with open(csv_filename, 'w', newline='', encoding='utf-8') as f:
            if data and isinstance(data[0], dict):  # Ensure data is not empty and is a list of dicts
...
def check_quality(text, prompt, existing_data):
    ...
    required_keywords = ["design", "create", "process", "steps", "project", "software"]  # Added more keywords
    ...
    density_threshold = 0.01  # Reduced from 0.02
    ...
    if existing_data:
        ...
        similarity_threshold = 0.9  # Increased from 0.8
...
def back_translate(text, target_lang='fr'):
    try:
        ...
        # Add a check for empty or None text
        if not text:
        ...
        time.sleep(1)  # Another delay before the second translation
...
def generate_creative_data(existing_data, num_samples=5, max_tokens=200, complexity="medium", word_embedding_model=None):
    new_data = []
    existing_hashes = set(item.get('hash', generate_hash(item['generated_text'])) for item in existing_data)
    max_attempts = num_samples * 3  # Increase the number of attempts
